...
class VideoApp:    
    def __init__(self, root):
        ...
        self.client = None  # Dodaj inicializacijo self.client
    ...
    def start_streaming(self):
        ...
        # Zaženi zajem v ločenem threadu
        ...
        self.portLED = 22345
        # Pošlji START sporočilo Unity-ju prek UDP
        if self.connected:
            self.udp_socket.sendto(b"START", (self.udp_ip, self.portLED))
            print("Sent START message to Unity.")

    def stop_streaming(self):
                # Pošlji STOP sporočilo Unity-ju prek UDP, če je povezava aktivna
        if self.connected:
            self.udp_socket.sendto(b"STOP", (self.udp_ip, self.portLED))
            print("Sent STOP message to Unity.")
        else:
            print("No active connection to Unity.")
        """Stop streaming the GUI capture."""
        self.start_button.config(state=tk.NORMAL)  # Re-enable Start button
        self.stop_stream_button.config(state=tk.DISABLED)  # Disable Stop button
