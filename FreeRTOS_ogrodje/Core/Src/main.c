...
{
    char receivedCommand[64];

    while (1) {
        if (xQueueReceive(cdcRxQueue, &receivedCommand, portMAX_DELAY) == pdPASS) {
            if (strcmp(receivedCommand, "ON") == 0) {
				HAL_GPIO_WritePin(GPIOD, GPIO_PIN_14, GPIO_PIN_SET);	// Prizgi rdeco LED

            } else if (strcmp(receivedCommand, "OFF") == 0) {
